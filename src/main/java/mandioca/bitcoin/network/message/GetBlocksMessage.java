...
import static mandioca.bitcoin.network.message.MessageType.GETBLOCKS;
 ...
 * <p>
 * The locator hashes are processed by a node in the order as they appear in the message. If a block hash is found
 * in the node's main chain, the list of its children is returned back via the inv message and the remaining locators
 * are ignored, no matter if the requested limit was reached, or not.
 * <p>
 * To receive the next blocks hashes, one needs to issue getblocks again with a new block locator object. Keep in
 * mind that some clients may provide blocks which are invalid if the block locator object contains a hash on the
 * invalid branch.
 *
 ...
 */
public final class GetBlocksMessage extends AbstractNetworkMessage implements NetworkMessage {

    // TODO this class is almost identical to GetHeadersMessage (refactor after much testing)
    ...
    /**
     * hash (32 bytes) of the last desired block header; set to zero to get as many blocks as possible (500)
     */
    private final byte[] hashStop;
    private final boolean hasZeroHashStop;

    public GetBlocksMessage(byte[] blockLocator) {
        this(DEFAULT_VERSION, DEFAULT_HASH_COUNT, blockLocator, null);
    }

    public GetBlocksMessage(String blockLocator) {
        this(DEFAULT_VERSION, DEFAULT_HASH_COUNT, blockLocator, null);
    }

    public GetBlocksMessage(byte[] blockLocator, byte[] hashStop) {
        this(DEFAULT_VERSION, DEFAULT_HASH_COUNT, blockLocator, hashStop);
    }

    public GetBlocksMessage(String blockLocator, String hashStop) {
        this(DEFAULT_VERSION, DEFAULT_HASH_COUNT, blockLocator, hashStop);
    }

    public GetBlocksMessage(byte[] version, byte[] hashCount, byte[] blockLocator, byte[] hashStop) {
    ...
    public GetBlocksMessage(byte[] version, byte[] hashCount, String blockLocator, String hashStop) {
        validateBlockLocatorHex.accept(blockLocator);
        this.messageType = GETBLOCKS;
        this.version = version;
        this.hashCount = hashCount;
    ...
    public static GetBlocksMessage parse(byte[] payload) {
        ...
        try {
            ...
            return new GetBlocksMessage(versionBytes, VARINT.encode(hashCount), blockLocator, hashStop);
        } catch (Exception e) {
